...
__status__ = "Dev"
__desc__ = "Utility functions for local quantization: normalization, schema validation, types, business rules and quality."
'''
...
def validate_schema(data: Dict[str, Any]) -> List[Dict]:
    """
        Validate required fields for quantization pipeline
...
def validate_types(data: Dict[str, Any]) -> List[Dict]:
    ...
    ## Bits type
    if "bits" in data and not isinstance(data["bits"], int):
        logger.error("Invalid bits type")
        issues.append({
            "rule": "type_bits",
            "level": "error",
            "message": "bits must be int",
        })
...
def check_business_rules(data: Dict[str, Any]) -> List[Dict]:
    """
        Apply quantization business rules
    ...
    ## Bits allowed
    if "bits" in data:
        if data["bits"] not in [2, 3, 4, 8]:
            logger.error("Invalid bits value")
            issues.append({
                "rule": "business_bits",
                "level": "error",
                "message": "bits must be in [2,3,4,8]",
            })
